...
class ConsoleGrid {
    ...
    setOption(option) {
        ...
        Object.keys(o).forEach(key => {
            ...
            // remove ascii chars (means pick up NON ascii chars)
            var chars = o[key].replace(/[\x00-\xff]+/g, '');
            nonAscIIs += chars;
        });

        // if nonAscIIs is empty, then the reg is null. Otherwise create a new RegExp.
        this.option.treeIconsReg = !nonAscIIs ? null : new RegExp('[' + nonAscIIs + ']', 'g');
    ...
    getColumnWidth(column) {
        var isFirstColumn = column.cg_index === 0;
        var w = 0;
        if (!this.option.hideHeaders) {
            w = this.getCharLength(column.name, isFirstColumn);
        }
        this.gridRows.forEach(row => {
            var str = row[this.getColumnKey(column)];
            var len = this.getCharLength(str, isFirstColumn);
            w = Math.max(w, len);
        });
    ...
    getHeaderLines(column) {
        ...
        var str = column.name;
        var lenNoColor = this.getCharLength(str, isFirstColumn);
        ...
        while (list.length) {
            var item = list.shift();
            while (list.length && this.getCharLength(item + list[0], isFirstColumn) < columnWidth) {
                item += " " + list.shift();
            }
            lines.push(item);
        }
    ...
    getCharByWidth(str, w, isFirstColumn) {
        ...
        var item = list.shift();
        //first one overflow
        var lenNoColor = this.getCharLength(item, isFirstColumn);
        if (lenNoColor > w) {
        ...
        while (list.length && this.getCharLength(item + list[0], isFirstColumn) < w - 4) {
            item += " " + list.shift();
        }

        //+1 space
        var lenCurrent = this.getCharLength(item, isFirstColumn);
    ...
    getCharLength(char, isFirstColumn) {
        //console.log(char, char.length);
        char = this.removeColor(char);
        //console.log(char, char.length);
        var len = this.getByteLen(char);

        // if the column is first, and the treeIconsReg is not null, then check the tree icons
        if (isFirstColumn && this.option.treeIconsReg) {
    ...
    getCell(column, str) {
        ...
        var lenNoColor = this.getCharLength(str, isFirstColumn);
        if (lenNoColor > columnWidth) {
            str = this.getCharByWidth(str, columnWidth, isFirstColumn);
            lenNoColor = this.getCharLength(str, isFirstColumn);
        }
